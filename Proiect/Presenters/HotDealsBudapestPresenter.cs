...
namespace Proiect.Presenters
{
    class HotDealsBudapestPresenter
    {
        ...
        public void Populate()
        {
            ...
            myCon.Open();
            SqlDataAdapter da = new SqlDataAdapter("SELECT ACTIVITY, ACCOMODATION_TYPE, REVIEW, CENTER_DISTANCE,MEALS_INCLUDED, NO_NIGHTS, ADULTS, CHILDREN, (ACTIVITATI.BUDGET+CAZARE.BUDGET) AS TOTAL FROM ACTIVITATI FULL OUTER JOIN CAZARE ON ACTIVITATI.ID=CAZARE.ID WHERE ACTIVITATI.ID=13 AND (ACTIVITATI.BUDGET+CAZARE.BUDGET) BETWEEN 200 AND 399", myCon);
